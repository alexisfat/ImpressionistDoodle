...
public class MainActivity extends AppCompatActivity implements OnMenuItemClickListener {
    ...
    public boolean onMenuItemClick(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.menuRectangle:
                Toast.makeText(this, "Rectangle Brush", Toast.LENGTH_SHORT).show();
                _impressionistView.setBrushType(BrushType.Rectangle);
                return true;
            case R.id.menuLine:
                Toast.makeText(this, "Line Brush", Toast.LENGTH_SHORT).show();
                _impressionistView.setBrushType(BrushType.Line);
                return true;
            case R.id.menuPopcorn:
                Toast.makeText(this, "Popcorn Brush", Toast.LENGTH_SHORT).show();
                _impressionistView.setBrushType(BrushType.Popcorn);
                return true;
    ...
    /*this function saves the current bitmap to the phone or tablet's gallery*/
        ...
        new AlertDialog.Builder(this)
                ...
                .setPositiveButton(android.R.string.yes, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int whichButton) {
                        ...
                        Bitmap bitmapCanvas = _impressionistView.getOffScreenBitmap();
                        String saveAs = UUID.randomUUID().toString() + ".jpg";
                        File file = new File (Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES), saveAs);
                        try {
                            bitmapCanvas.compress(Bitmap.CompressFormat.PNG, 100, new FileOutputStream(file));
                            FileUtils.addImageToGallery(file.getAbsolutePath(), getApplicationContext());
